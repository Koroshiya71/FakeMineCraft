...
namespace Uniblocks
{
    public class ChunkManager : MonoBehaviour
    {
        ...
        private static List<Chunk>
            ChunkUpdateQueue; // stores chunks ordered by update priority. Processed in the ProcessChunkQueue loop
        ...
        public static bool
            StopSpawning; // when true, the current SpawnChunks sequence is aborted (and this is set back to false afterwards)
        ...
        void Start()
        {
            ...
            ChunkObject.GetComponent<Chunk>().MeshContainer.transform.localScale =
                ChunkObject.transform.localScale; // set correct scale of trigger collider and additional mesh collider
        ...
        public static GameObject SpawnChunkFromServer(Index index)
        {
            ...
            if (chunk == null)
            {
                ...
                return
                    chunk; // don't disable mesh generation and don't enable timeout for chunks that are already spawned
        }
        ...
        public void Update()
        {
            ...
            if (!SpawningChunks && !ProcessChunkQueueLoopActive && ChunkUpdateQueue != null &&
                ChunkUpdateQueue.Count > 0)
            {
        ...
        private IEnumerator SpawnMissingChunks(int originX, int originY, int originZ, int range)
        {
            ...
            ChunkUpdateQueue =
                new List<Chunk>(); // clear update queue - it will be repopulated again in the correct order in the following loop

            // flag chunks not in range for removal
            ChunksToDestroy = new List<Chunk>();
            foreach (Chunk chunk in Chunks.Values)
            {
                if (Vector2.Distance(new Vector2(chunk.ChunkIndex.x, chunk.ChunkIndex.z),
                    new Vector2(originX, originZ)) > range + Engine.ChunkDespawnDistance)
                {
                                ...
                                if (Mathf.Abs(originX - x) + Mathf.Abs(originZ - z) < range * 1.3f)
                                {
                                    ...
                                    if (currentChunk != null)
                                    {
                                        ...
                                        if (currentChunk.Fresh)
                                        {
                                            ...
                                            {
                                                Index neighborIndex =
                                                    currentChunk.ChunkIndex.GetAdjacentIndex((Direction) d);
                                                GameObject neighborChunk = GetChunk(neighborIndex);
                                                if (neighborChunk == null)
                                                {
                                                    neighborChunk = Instantiate(ChunkObject, neighborIndex.ToVector3(),
                                                        transform.rotation) as GameObject;
                                                }

                                                currentChunk.NeighborChunks[d] =
                                                    neighborChunk
                                                        .GetComponent<Chunk
                                    ...
                                    else
                                    {
                                        ...
                                        // spawn chunk
                                        GameObject newChunk =
                                            Instantiate(ChunkObject, new Vector3(x, y, z),
                                                transform.rotation) as GameObject; // Spawn a new chunk.
