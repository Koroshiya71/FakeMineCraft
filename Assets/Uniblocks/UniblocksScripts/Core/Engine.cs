...
// enums
    ...
    public class Engine : MonoBehaviour
    {
        ...
        public static bool ShowBorderFaces,
            GenerateColliders,
            SendCameraLookEvents,
            SendCursorEvents,
            EnableMultiplayer,
            MultiplayerTrackPosition,
            SaveVoxelData,
            GenerateMeshes;

        public bool lShowBorderFaces,
            lGenerateColliders,
            lSendCameraLookEvents,
            lSendCursorEvents,
            lEnableMultiplayer,
            lMultiplayerTrackPosition,
            lSaveVoxelData,
            lGenerateMeshes;
        ...
        public void Awake()
        {
            ...
            {
                Debug.LogWarning(
                    "Uniblocks: Layer 26 is reserved for Uniblocks; it is automatically set to ignore collision with all layers.");
            }
            ...
            {
                Debug.LogError(
                    "Uniblocks: The blocks array is empty! Use the Block Editor to update the blocks array.");
                Debug.Break();
            }
            ...
            if (Engine.HeightRange < 0)
            {
                ...
                Debug.LogWarning(
                    "Uniblocks: Chunk height range can't be a negative number! Setting chunk height range to 0.");
            }
            ...
            if (Engine.MaxChunkDataRequests < 0)
            {
                ...
                Debug.LogWarning(
                    "Uniblocks: Max chunk data requests can't be a negative number! Setting max chunk data requests to 0.");
            }
            ...
            for (ushort i = 0; i < Engine.Blocks.Length; i++)
            {
                if (Engine.Blocks[i] != null)
                {
                    ...
                    {
                        Debug.LogError("Uniblocks: Voxel " + i +
                                       " has a material index lower than 0! Material index must be 0 or greater.");
                        Debug.Break();
                    }

                    if (voxel.VSubmeshIndex > materialCount)
                    {
                        Debug.LogError("Uniblocks: Voxel " + i + " uses material index " + voxel.VSubmeshIndex +
                                       ", but the chunk prefab only has " + (materialCount + 1) +
                                       " material(s) attached. Set a lower material index or attach more materials to the chunk prefab.");
                        Debug.Break();
                    }
                }
            }
        ...
        public static void SavePlayerPos()
        {
            ...
            GameTool.SetFloat("PlayerPos.X", playerPos.x);
            GameTool.SetFloat("PlayerPos.Y", playerPos.y);
            GameTool.SetFloat("PlayerPos.Z", playerPos.z);
        }
        // ==== other ====	
            ...
            if (Physics.Raycast(origin, direction, out hit, range))
            {
                    ...
                    || hit.collider.GetComponent<ChunkExtension>() != null)
                {
                    ...
                    if (hitObject.GetComponent<ChunkExtension>() != null)
                    {
                        ...
                        hitObject = hitObject.transform.parent
                            .gameObject; // swap the mesh container for the actual chunk object
                    }
                    ...
                    return new VoxelInfo(
                        hitObject.GetComponent<Chunk>()
                            .PositionToVoxelIndex(hit.point, hit.normal, false), // get hit voxel index
                        hitObject.GetComponent<Chunk>()
                            .PositionToVoxelIndex(hit.point, hit.normal, true), // get adjacent voxel index
        ...
        public static Vector2 GetTextureOffset(ushort voxel, Facing facing)
        {
            ...
            if (textureArray.Length == 0)
            {
                ...
                Debug.LogWarning("Uniblocks: Block " + voxel.ToString() +
                                 " has no defined textures! Using default texture.");
